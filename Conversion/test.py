...
import datetime
connection = None
nusc = NuScenes(version='v1.0-mini', dataroot = 'D:\\edge\\v1.0-mini', verbose = True)
...
# 插入annotation数据
def insert_annotation(connection):
    ...
    # 插入数据到annotation表
    for item in nusc.annotation:
        token = item["token"]
        instance_token = item["instance_token"]
        attribute_tokens = item["attribute_tokens"]
        visibility_token = item["visibility_token"]
        translation_x, translation_y, translation_z = item["translation"]

        try:
            # 执行插入SQL语句
            insert_query = """
            INSERT INTO annotation (
                token, instance_token, attribute_tokens, visibility_token,
                translation_x, translation_y, translation_z
            ) VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(insert_query, (token, instance_token, ','.join(attribute_tokens),
                                          visibility_token, translation_x, translation_y, translation_z))
        except mysql.connector.Error as e:
            print(f"Error inserting data into annotation: {e}")

    # 提交事务
    connection.commit()
    print("annotation表数据成功插入！")
    cursor.close()
