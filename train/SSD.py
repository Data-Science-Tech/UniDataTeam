...
class DatabaseDataset(Dataset):
    def __init__(self, db_config, log_info_id, is_training=True):
        ...
        self.cursor.execute("""SELECT sensor_calibration_id FROM log_info WHERE log_info_id = %s""", (self.log_info_id,))
        ...
        self.cursor.execute("""SELECT sensor_data_id, image_resolution FROM sensor_data WHERE sensor_calibration_id = %s""", (self.sensor_calibration_id,))
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training and self.car_category_id is not None:
            self.cursor.execute("""SELECT 2d_box_xmin, 2d_box_xmax, 2d_box_ymin, 2d_box_ymax FROM sample_annotation JOIN sample_info ON sample_annotation.sample_id = sample_info.sample_id WHERE sample_info.sensor_data_id = %s AND category_description_id = %s""", (sensor_data_id, self.car_category_id))
